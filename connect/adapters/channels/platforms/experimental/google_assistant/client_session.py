"""
Google Assistant-specific auth helper.

Returns a (client, error_message) tuple::

    client, err = get_backend_client(session_id)
    if err:
        await say(err)
        return
"""
...
def get_backend_client(user_id: str) -> Tuple[Optional[OpenJiuwenClient], Optional[str]]:
    ...
    if not token:
        return None, "You are not logged in. Say: login"
    ...
    if not ok:
        ...
        return None, "Your session has expired. Say: login"
