...
async def handle_login_username(user_id: str, text: str, say) -> None:
    ...
    username = text.strip()
    if not username:
        await say("Username cannot be empty. Please reply with your username.")
        return
...
async def _do_login(user_id: str, username: str, password: str, say) -> None:
    ...
    result = await _run(lambda: do_login(client, username, password))
    ud = get_user_data(user_id)
    if result.get("error"):
        ud.pop("state", None)
    ...
    ud.pop("state", None)
    ud.pop("pending_username", None)
    await say("You are now logged in.")
