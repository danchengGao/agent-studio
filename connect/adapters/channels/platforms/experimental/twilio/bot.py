"""Route inbound SMS text to the appropriate handler."""
...
async def handle_message(user_id: str, text: str, say) -> None:
    """Dispatch an inbound SMS to the correct handler."""
    ud = get_user_data(user_id)
    text = text.strip()

    # Multi-turn flows take priority
    if ud.get("state") == "awaiting_username":
        await handle_login_username(user_id, text, say)
        return
    if ud.get("state") == "awaiting_password":
        await handle_login_password(user_id, text, say)
        return
    if ud.get("workflow_session"):
        low = text.lower()
        if low in ("cancel", "workflow cancel"):
            await handle_workflow_cancel(user_id, text, say)
        elif low == "skip":
            await handle_workflow_collect(user_id, "SKIP", say)
        else:
            await handle_workflow_collect(user_id, text, say)
        return
    ...
    if cmd == "login":
        await handle_login_start(user_id, text, say)
    elif cmd == "logout":
        await handle_logout(user_id, text, say)
    elif cmd == "status":
        await handle_status(user_id, text, say)
    elif cmd == "cancel":
        await handle_cancel(user_id, text, say)
    elif cmd == "health":
        await handle_health(user_id, text, say)
    elif cmd in ("help", "start"):
        await handle_help(user_id, text, say)
    elif cmd == "workflows" and not sub:
        await handle_workflows_list(user_id, text, say)
        ...
        await handle_agents_list(user_id, text, say)
        ...
        await say(f"Unknown command: {text!r}. Send 'help' to see available commands.")
