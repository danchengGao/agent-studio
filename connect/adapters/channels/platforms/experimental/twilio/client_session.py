"""Auth helper for the Twilio platform."""
...
async def get_backend_client(user_id: str, say):
    """Return an authenticated OpenJiuwenClient or (None, error_str)."""
    ...
    if not token_data:
        await say("Please log in first. Send: login")
    ...
    if not ok:
        await say("Session expired. Please log in again. Send: login")
