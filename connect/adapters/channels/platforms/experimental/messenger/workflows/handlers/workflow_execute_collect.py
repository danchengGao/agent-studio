...
async def handle(user_id: str, text: str, say, user_data: dict) -> None:
    ...
    if wf is None:
        ...
        await say("No active workflow. Send 'workflows' to start.")
        return
    ...
    # Find next non-optional param
    while next_idx < len(params):
        next_param = params[next_idx]
        next_name = next_param.get('name', 'value')
        next_desc = next_param.get('description') or next_param.get('desc', '')
        msg = f"Enter value for '{next_name}'"
        if next_desc:
            msg += f" ({next_desc})"
        await say(msg)
        return
