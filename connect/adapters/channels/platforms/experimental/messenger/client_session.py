"""
Messenger-specific auth helper.

Returns a (client, error_message) tuple.  The caller awaits the error
send itself when needed::
...
def get_backend_client(user_id: str) -> Tuple[Optional[OpenJiuwenClient], Optional[str]]:
    ...
    if not token:
        return None, "Not logged in. Send 'login' to authenticate."
    ...
    if not ok:
        ...
        return None, "Session expired. Send 'login' to authenticate again."
