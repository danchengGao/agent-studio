"""
Teams-specific auth helper.

Teams handlers are async (botbuilder uses aiohttp), so the respond-callback
pattern used by Slack doesn't translate cleanly.  Instead this module returns
a (client, error_message) tuple — the async handler awaits the error send
itself when needed.
"""
...
def get_backend_client(user_id: str) -> Tuple[Optional[OpenJiuwenClient], Optional[str]]:
    ...
    if not token:
        return None, "❌ Not logged in. Type `login` to authenticate."
    ...
    if not ok:
        ...
        return None, "❌ Session expired. Type `login` to authenticate again."
