from connect.client import do_login
...
def _do_login(user_id: str, username: str, password: str, say, user_data: dict) -> None:
    ...
    try:
        ...
        say(f"✅ Logged in as `{username}`! Use `/logout` to sign out.")
    except Exception as e:
        user_data['state'] = 'idle'
        say(f"❌ Login failed: {e}\n\nUse `/login` to try again.")
