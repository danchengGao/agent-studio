...
sed -i '/^CODE_SANDBOX_URL=/d; /^DB_HOST=/d; /^MILVUS_HOST=/d; /^SERVER_AES_MASTER_KEY=/d; /^SSL_PASSWORD=/d; /^VITE_API_PROXY_TARGET=/d; /^VITE_PLUGIN_SERVICE_URL=/d; /^MINIO_HOST=/d; /^DEEPSEARCH_AGENT_HOST=/d; /^RUNTIME_HOST=/d' .env.runtime.default
