...
# Default app DB account
$APP_DB_USER = "openjiuwen"
$APP_DB_PASSWORD = "openjiuwen2026"
...
# Write DB user/password to .env (same as Linux config_mysql.sh)
function Update-EnvFileDbCredentials {
    param(
        [string]$User,
        [string]$Password
    )
    $EnvDir = Split-Path -Parent $ENV_FILE
    if (-not (Test-Path $EnvDir)) {
        New-Item -ItemType Directory -Path $EnvDir -Force | Out-Null
    }
    if (Test-Path $ENV_FILE) {
        $BackupPath = "${ENV_FILE}.bak.$(Get-Date -Format 'yyyyMMddHHmmss')"
        Copy-Item -Path $ENV_FILE -Destination $BackupPath -Force -ErrorAction SilentlyContinue
    }
    $Content = @()
    $UserWritten = $false
    $PasswordWritten = $false
    if (Test-Path $ENV_FILE) {
        foreach ($Line in Get-Content -Path $ENV_FILE -Encoding UTF8) {
    ...
    if (-not $UserWritten) { $Content += "DB_USER=$User" }
    if (-not $PasswordWritten) { $Content += "DB_PASSWORD=$Password" }
    $Content | Set-Content -Path $ENV_FILE -Encoding UTF8 -NoNewline:$false
    Write-Log 'SUCCESS' ('.env updated: DB_USER=' + $User)
}
...
function Create-MySQLDatabases {
    ...
    Write-Log 'INFO' ('OPS Database: ' + $OPS_DB_NAME + ', AGENT Database: ' + $AGENT_DB_NAME)
    Write-Log 'INFO' ('App account: ' + $APP_DB_USER + ' (will grant and write to .env after DB creation)')
    ...
    $SqlCreateDbs = $CreateOPSDB + "`n" + $CreateAGENTDB

    $CheckUserSql = "SELECT COUNT(*) FROM mysql.user WHERE user='$APP_DB_USER' AND host='localhost';"
    $GrantSql = 'GRANT ALL PRIVILEGES ON ' + $OPS_DB_NAME + '.* TO ''' + $APP_DB_USER + '''@''localhost''; GRANT ALL PRIVILEGES ON ' + $AGENT_DB_NAME + '.* TO ''' + $APP_DB_USER + '''@''localhost''; FLUSH PRIVILEGES;'
    ...
    Write-Log 'SUCCESS' ('Databases ' + $OPS_DB_NAME + ', ' + $AGENT_DB_NAME + ' created or already exist')
    ...
    Write-Log 'SUCCESS' ('Granted ' + $APP_DB_USER + ' on ' + $OPS_DB_NAME + ', ' + $AGENT_DB_NAME)
    
    # Write default app account to .env
    Update-EnvFileDbCredentials -User $APP_DB_USER -Password $APP_DB_PASSWORD
    ...
    if ($VerifyR.ExitCode -eq 0 -and $VerifyR.Result -match $OPS_DB_NAME -and $VerifyR.Result -match $AGENT_DB_NAME) {
        Write-Log 'SUCCESS' ('App account can access DB: ' + $OPS_DB_NAME + ', ' + $AGENT_DB_NAME)
        return $true
    }
...
function Start-MySQLDatabaseConfig {
    ...
    Write-Log 'INFO' ('Creating databases: ' + $MYSQL_OPS_DB_NAME + ' and ' + $MYSQL_AGENT_DB_NAME)
    Write-Log 'INFO' ('Connection details: ' + $APP_DB_USER + '@' + $MYSQL_HOST + ':' + $MYSQL_PORT)
    ...
    if ($Result) {
        ...
        Write-Log 'INFO' ('Please verify that databases ' + $MYSQL_OPS_DB_NAME + ' and ' + $MYSQL_AGENT_DB_NAME + ' exist')
        return $false
    }
}
