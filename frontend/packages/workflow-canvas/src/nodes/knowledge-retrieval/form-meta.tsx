...
function KnowledgeRetrievalKBList({ onKBChange }: { onKBChange?: () => void } = {}) {
  ...
  const [selectorKbIds, setSelectorKbIds] = useState<string[]>([]);
  ...
  const fetchKnowledgeBases = useCallback(
    async (notifyChange = false) => {
      ...
      try {
        ...
        if (response?.data?.items) {
          ...
          if (kbIds.length > 0) {
            const updatedList = kbIds.map(
              id =>
                kbs.find((kb: KnowledgeBaseInfo) => kb.id === id) || {
                  id,
                  name: id,
                  description: '',
            ...
            updateKnowledgeRetrievalParam(formModel, {
              kbInfo: updatedList.map(k => ({
                id: k.id,
                name: k.name,
                description: k.description,
                has_graph_enhancement: k.has_graph_enhancement,
                ds_kb_id: k.ds_kb_id,
              })),
            });
            if (notifyChange) {
    ...
    [formModel, kbIds, onKBChange],
  );

  const handleAdd = () => {
    setSelectorKbIds(kbList.map(kb => kb.id));
    fetchKnowledgeBases();
    setShowSelector(true);
  };

  const applyKBSelection = useCallback(
    (newList: KnowledgeBaseInfo[]) => {
      setKbList(newList);
      const hasGraph = newList.some(kb => kb.has_graph_enhancement);
      updateKnowledgeRetrievalParam(formModel, {
        kbIds: newList.map(k => k.id),
        kbInfo: newList.map(k => ({
          id: k.id,
          name: k.name,
          description: k.description,
          has_graph_enhancement: k.has_graph_enhancement,
    ...
    [formModel, onKBChange],
  );

  const handleToggleKB = (kb: KnowledgeBaseInfo) => {
    setSelectorKbIds(prev => {
  ...
  const handleSaveSelection = () => {
    const selectedList: KnowledgeBaseInfo[] = selectorKbIds.map(id => {
      return (
        availableKBs.find(kb => kb.id === id) ||
        kbList.find(kb => kb.id === id) || {
          id,
          name: id,
          description: '',
          has_graph_enhancement: false,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {kbList.map(kb => {
            ...
            return (
              ...
              >
                <div
                  style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden', flex: 1, minWidth: 0 }}
                >
                  <span
                    style={{
                      display: 'flex',
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: 16 }}>
                {t('workflowCanvas.nodes.knowledgeRetrieval.selectKnowledgeBase')}
              </h3>
              <button
                onClick={handleCancelSelection}
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {availableKBs.map(kb => {
                  const isSelected = selectorKbIds.includes(kb.id);
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16, gap: 8 }}>
              ...
              >
                {t('workflowCanvas.nodes.knowledgeRetrieval.cancel') || 'Cancel'}
              </button>
              <button
              ...
              >
                {t('workflowCanvas.nodes.knowledgeRetrieval.save') || 'Save'}
              </button>
            </div>
          </div>
        </div>
  ...
  useLayoutEffect(() => {
    if (show && tooltipRef.current && triggerRef.current) {
      ...
      setPos({
        ...
        left: Math.max(
          8,
          Math.min(triggerRect.left + triggerRect.width / 2 - tooltipWidth / 2, window.innerWidth - tooltipWidth - 8),
        ),
      });
    }
  }, [show]);
    ...
    >
      ...
      {show && (
        ...
        >
          ...
          {plain ? (
            <div style={{ fontSize: 12, color: '#4e5969', lineHeight: 1.6 }}>{steps.map(s => s.content).join(' ')}</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {steps.map((step, idx) => (
                <div
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
          ...
          <RetrievalTooltip
            ...
            plain
            steps={[{ label: '', content: t('workflowCanvas.nodes.knowledgeRetrieval.maxRecallTooltip') }]}
          />
        </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
          ...
          <RetrievalTooltip
            ...
            plain
            steps={[{ label: '', content: t('workflowCanvas.nodes.knowledgeRetrieval.minScoreTooltip') }]}
          />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <input
      ...
      <div style={{ marginBottom: 12, opacity: hasGraphEnhancement ? 1 : 0.5 }}>
        ...
        >
          ...
          <RetrievalTooltip
            ...
            steps={[
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipQuestion'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.graphExampleQuestion'),
              },
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipGraphExpansion'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.graphExampleExpansion'),
              },
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipAnswer'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.graphExampleAnswer'),
              },
            ]}
          />
        </label>
        ...
        >
          ...
          <RetrievalTooltip
            ...
            steps={[
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipQuestion'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.agenticExampleQuestion'),
              },
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipStep1'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.agenticExampleStep1'),
              },
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipStep2'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.agenticExampleStep2'),
              },
              {
                label: t('workflowCanvas.nodes.knowledgeRetrieval.tooltipAnswer'),
                content: t('workflowCanvas.nodes.knowledgeRetrieval.agenticExampleAnswer'),
              },
            ]}
          />
        </label>
  ...
  return (
    <>
      ...
      <FormContent>
        ...
        <KnowledgeSection />
        <FormOutput
          name="workflowCanvas.formOutput.output"
          outputName="outputs"
          showAddButton={false}
          readonly={true}
        />
