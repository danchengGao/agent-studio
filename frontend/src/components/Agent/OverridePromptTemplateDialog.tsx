    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        <div className="flex items-center gap-2">
          ...
          <span>{t('agents.agentEditor.systemPrompt.promptDialog.goToTemplateManagement')}</span>
        </div>
      </DialogTitle>
      <DialogContent>
        <div className="space-y-3">
          <Typography variant="body2">{t('agents.agentEditor.systemPrompt.overwritePromptTemplateDialog.description')}</Typography>
          <Alert severity="warning">
            <div className="text-sm">
              <div className="font-medium">操作说明：</div>
              <ul className="list-disc list-inside mt-1">
                <li>{t('agents.agentEditor.systemPrompt.overwritePromptTemplateDialog.overwriteAndJump')}</li>
                <li>{t('agents.agentEditor.systemPrompt.overwritePromptTemplateDialog.notOverwriteAndJump')}</li>
              </ul>
            </div>
          </Alert>
        </div>
      ...
      <DialogActions>
        <Button onClick={onClose}>{t('agents.agentEditor.systemPrompt.overwritePromptTemplateDialog.cancel')}</Button>
